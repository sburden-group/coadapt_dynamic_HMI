...
# AMBER: GRID SEARCH FOR 2nd order G's parameters  (b0z**2 + b1*z + b2) / (z**2 + a0*z + a1)
with open('protocols/global_search_interfaces_2norm_cost.pkl', 'rb') as file:
    ...
    _,_,G_init, _,_,G_find = pickle.load(file)
...
if self_defined_init:
  user_input = input("Type in the last machine (eg 0 0 0 0 1); press enter to randomize: ")
  if user_input == "":
      import datetime
  ...
  if passthrough: 
      G_est = np.array([0,0,0,0,1]) #baseline interface = 1
  else:
...
def loss(H, G):
    ...
    G_ = second(G) # I = (b0z**2 + b1*z + b2) / (z**2 + a0*z + a1)
    # DIST = D[IX]
  ...
  if not PAUSE:
    ...
    if TRIAL_STATE in ['run','reset0','reset1','reset2','react']:
      ...
      if TRIAL_STATE in 'run':
        a0,a1,b0,b1,b2 = G_est[0],G_est[1],G_est[2],G_est[3],G_est[4]
        G_est_ = np.zeros((int(len(G_est)),))
        # denominator = z^2 + a0z + a1
        G_est_[0] = a0 
        G_est_[1] = a1
        # numerators = b0z^2 + b1z + b2 
        G_est_[2] = b0
        G_est_[3] = b1
        G_est_[4] = b2
        ...
        _mach = dynamics.higher_order_machine(t=_time,uH=inp_[-m2:],uG=mach_[-n2:],G=G_est_,n=n2,m=m2, order = 2) # machine adapts user input #Amber: added order
        if len(out_)<2:
            y = 0#_mach
        else:
