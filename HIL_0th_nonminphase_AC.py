...
def save(sfx='',csv=True,**trial_data):
  ...
  if csv:
    ...
    if 'H_' in trial_data.keys():
      ...
      Gsave = np.ones(np.asarray(time).shape)*trial_data['G_']
      mach = np.asarray(trial_data['mach_'])[-len(inp):]
      d = np.vstack((time[:2701],realtime[:2701],ref[:2701],inp[:2701],dis[:2701],state.T[:2701],mach[:2701],Hsave[:2701],Gsave[:2701])).T
...
# AMBER: GRID SEARCH FOR 0th order G's parameters (gain = b)
...
with open('protocols/global_search_interfaces_2norm_cost.pkl', 'rb') as file:
    ...
    G_init,_,_, G_find,_,_ = pickle.load(file)
...
if self_defined_init:
  user_input = input("Type in the last machine (eg 1); press enter to randomize: ")
  if user_input == "":
      import datetime
...
while not done:
  ...
  if not PAUSE:
    ...
    if TRIAL_STATE == 'run' and _time + SHIP_SHIFT > trial['duration']:
      ...
      T = int(2400) # length of time points - CHANGE TO 2400
    ...
    if TRIAL_STATE in ['run','reset0','reset1','reset2','react']:
      ...
      if TRIAL_STATE in 'run':
        # AMBER: updated 0th order interface output uI[t] = constant G [t] * human input [t], both at current time
        _mach = dynamics.gain_machine(t=_time,uH=_inp,uG=0,G=G_est,n=n2,m=m2) # machine adapts user input, machine is just a gain
        if len(out_)<3:
            y = 0#_mach
        else:
